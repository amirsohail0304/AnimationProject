...
import { initialHorRadius, initialSideWidth, initialVertRadius, maxHorRadius, maxVertRadius } from '../../helpers/constants';
  ...
  export default function Weave({ centerY, progress, isBack, children }: any) {
    const sideWidth = useDerivedValue(() => {
      ...
      return (
        initialSideWidth +
        ((width - initialSideWidth) * (progress.value - p1)) / (p2 - p1)
      );
    });
    ...
    const vertRadius = useDerivedValue(() => {
      ...
      return (
        initialVertRadius +
        ((maxVertRadius - initialVertRadius) * progress.value) / p1
      );
    });
